...
proc push_work_to_fpga {workl} {
	...
	if { $testmode } {
		...
		set data3_nonce [expr 0x$data3_nonce - 2500000]
	...
	# write_instance "DAT1" $midstate
	write_instance "DAT1" $revmidstate
	write_instance "DAT2" $data3
...
proc check_if_fpga_is_miner {hardware_name device_name} {
	...
	if {[instance_exists DAT1] && [instance_exists DAT2] && [instance_exists NONC] && [instance_exists GNON]} {
		return 1
	}
