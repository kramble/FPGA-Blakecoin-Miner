...
dropdown menu in the middle left of the page, select the "tags" tab and scroll
down to the version required and select it. Download the zip (right side of page)
...
and rpcpassword set int your blakecoin.conf. Change localhost if running blakecoind on
a separate machine. You will need to install the WinUSB driver using zadig which is
available at http://sourceforge.net/projects/libwdi/files/zadig/

For linux users, run as follows (this is the same as RUNBLAKE.BAT)
./cgminer --disable-gpu --url localhost:8772 --userpass username:password 2>log.txt
...
same algorithm as bitcoin. It can be overriden byt the --ztex-clock option as follows
